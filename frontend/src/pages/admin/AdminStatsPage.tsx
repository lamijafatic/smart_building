...
import { Building2, Zap, CalendarDays, MapPin, Home, BellOff, SlidersHorizontal } from 'lucide-react';
import { buildingsApi } from '../../api/buildings';
import type { BuildingOverview } from '../../api/buildings';
import { adminApi } from '../../api/admin';
import type { AdminBuilding } from '../../api/admin';
...
export function AdminStatsPage() {
  ...
  const [adminBuildings, setAdminBuildings] = useState<AdminBuilding[]>([]);
  ...
  // Load overview when building selection changes
  useEffect(() => {
    if (selectedBuildingId == null) return;
    setLoading(true);
    setOverview(null);
    buildingsApi.getOverview(selectedBuildingId)
      .then((data) => {
        setOverview(data);
        setError(null);
      })
      .catch(() => {
        setError('Failed to load building statistics.');
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  if (adminBuildings.length === 0) {
    return (
      <div className="text-center py-16 text-slate-400 dark:text-white/30">
        ...
        <p>No buildings available. Create a building first.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Building Stats</h1>
        ...
        <div className="flex items-center gap-3">
          {adminBuildings.length > 1 && (
            <select
              ...
              className="rounded-xl bg-white dark:bg-white/[0.06] border border-slate-200 dark:border-white/[0.1] text-slate-900 dark:text-white px-3 py-2 text-sm focus:outline-none focus:border-yellow-400 dark:focus:border-yellow-400/60 transition"
            >
